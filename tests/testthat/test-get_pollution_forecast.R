...
with_mock_api({
  test_that("Function chart contains wrong data", {
    chart <- get_pollution_forecast(lat = 50.0,
                                  lon = 50.0,
                                  api_key = "api_key")
    expect_equal(colnames(chart$data), c("dt",
                                         "main.aqi",
                                         "Pollutants",
                                         "Concentration"))
    ...
    expect_equal(chart$labels$title,
  ...
  test_that("Function returns a failing API error", {
    expect_equal(get_pollution_forecast(lat = 50.0 ,
                                        lon = 50.0,
  ...
  test_that("Errors are thrown with incorrect inputs", {
    expect_error(get_pollution_forecast("49.28",
                                       123.12,
                                       "api_key"),
